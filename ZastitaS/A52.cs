...
namespace ZastitaS
{
    class A52
    {
		...
		public ShiftRegister W = new ShiftRegister();

		public A52()
		{
			this.SetDefaultKey();
			X.OperationMembers = new int[] { 12, 14, 15 };
			Y.OperationMembers = new int[] { 9, 13, 16 };
			Z.OperationMembers = new int[] { 13, 16, 18 };
			W.OperationMembers = new int[] { 3, 7, 10 };
		}

		public A52(string newKey) : this()
		{
			this.Key = newKey;
		}
		...
		public string Crypt(string source)
		{
			...
			foreach (char c in tmp)
			{
				...
				if (X.Register[W.Register[3]] == m)
					RegisterSteps(X);

				if (Y.Register[W.Register[7]] == m)
					RegisterSteps(Y);

				if (Z.Register[W.Register[10]] == m)
					RegisterSteps(Z);
